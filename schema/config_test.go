...
func TestLoadConfigAcceptsOptionalProtocolConstantsPath(t *testing.T) {
	dir := t.TempDir()
	if err := os.WriteFile(filepath.Join(dir, "golem.yaml"), []byte("protocol_constants: schemas/protocol_constants.yaml\nproto:\n  out: entities.proto\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	if got, want := cfg.ProtocolConstants, "schemas/protocol_constants.yaml"; got != want {
		t.Fatalf("ProtocolConstants = %q, want %q", got, want)
	}
}
