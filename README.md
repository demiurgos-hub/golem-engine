...
# Phaser package
cd golem-phaser && npm run build
```

The nested modules under `golem/collision/` and `golem/nav/` are developed with a local `go.work`. Do not commit `go.work`, `go.work.sum`, or generated `dist/` output.
