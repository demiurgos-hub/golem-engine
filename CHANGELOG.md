...
- Golem Scribe (Unity) Phase 4: mark prefabs with exactly one root `GolemFootprint` (optional unique alias + included layer mask) to export collision footprints to project-root `footprints.golem.yaml` (configurable). Entity status alone does not export colliders. Export includes enabled colliders on inactive children and skips disabled collider components; unsupported colliders inside the included mask fail that prefab instead of writing incomplete geometry. Hierarchy scale and quarter-turn limits match the Go placer (2D Circle/Box; 3D Sphere/Box). Footprint changes use the shared Scribe manifest/scheduler and never invoke `golem-bake`.
